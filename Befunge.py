...
quotes=Instruction('"',"String mode","Toggle string mode")
@quotes.runner
...
debug=Instruction('D',"Debug","Print 'Debug' to stdout")
@debug.runner
def run(instr,funge,pointer):
...
trampoline=Instruction('j',"Jump forward","Move [popped value]+1 times")
@trampoline.runner
def run(instr,funge,pointer):
...
end=Instruction('q',"Quit","Kill program. Exit code will be popped value")
@end.runner
def run(instr,funge,pointer):
...
iterate=Instruction('k',"Iterate","Execute next instruction [popped value] times")
@iterate.runner
def run(instr,funge,pointer):
...
def pushInstr(char,name,description,data):
	pusher=Instruction(char,name,description)
	@pusher.runner
	def run(instr,funge,pointer):
		pointer.pos+=pointer.delta
...
# Decision making
lnot=Instruction('!',"Not","Logical not - pop a value and push 1 if val==0 else 0")
@lnot.runner
def run(instr,funge,pointer):
...
gt=Instruction('`',"Greater Than","Pops b then a. Pushes one if a>b, else zero.")
@gt.runner
def run(instr,funge,pointer):
	a=pointer.stackPop()
	b=pointer.stackPop()
...
ewIf=Instruction('_',"East West If","Pops a value, acts like > if zero, else <")
@ewIf.runner
def run(instr,funge,pointer):
	n=pointer.stackPop()
	if n==0:
...
nsIf=Instruction('|',"North South If","Pops a value, acts like v if zero, else ^")
@nsIf.runner
def run(instr,funge,pointer):
	n=pointer.stackPop()
	if n==0:
...
nsIf=Instruction('w',"Compare","Pops b then a. if a<b, act like [, if a>b, act like ], no effect if a==b.")
@nsIf.runner
def run(instr,funge,pointer):
	b=pointer.stackPop()
	a=pointer.stackPop()
...
def simpleOpInstr(symbol,name,func):
	op=Instruction(symbol,name,f"Pops b then a. Pushes a{symbol}b.")
	@op.runner
	def run(instr,funge,pointer):
		b=pointer.stackPop()
		a=pointer.stackPop()
...
# Stack manipulation
pop=Instruction('$',"Pop","Pop a value and discard it")
@pop.runner
def run(instr,funge,pointer):
	pointer.stackPop()
	pointer.pos+=pointer.delta
...
pop=Instruction(':',"Duplicate","Pop a value and push it twice, duplicating it")
@pop.runner
def run(instr,funge,pointer):
	value=pointer.stackPop()
...
swap=Instruction('\\',"Swap","Swap the top two values on the stack")
@swap.runner
def run(instr,funge,pointer):
	a=pointer.stackPop()
	b=pointer.stackPop()
...
clear=Instruction('n',"Clear","Clear the stack")
@clear.runner
def run(instr,funge,pointer):
	pointer.stack=[]
	pointer.pos+=pointer.delta
...
# Editing FungeSpace
put=Instruction('p',"Put","Pops y, then x, then c. Places ascii code c at (x,y)+storage offset")
@put.runner
def run(instr,funge,pointer):
	y=pointer.stackPop()
	x=pointer.stackPop()
...
get=Instruction('g',"Get","Pops y, then x. Pushes ascii value of character at (x,y)+storage offset")
@get.runner
def run(instr,funge,pointer):
	y=pointer.stackPop()
	x=pointer.stackPop()
...
outNum=Instruction('.',"Output Decimal","Pops and prints the number.")
@outNum.runner
def run(instr,funge,pointer):
	n=pointer.stackPop()
	OUTNUM(n)
...
outChar=Instruction(',',"Output Character","Pops and prints the character.")
@outChar.runner
def run(instr,funge,pointer):
	c=pointer.stackPop()
	OUTCHAR(c)
...
inNum=Instruction('&',"Input Decimal","Pushes number from user input.")
@inNum.runner
def run(instr,funge,pointer):
...
inNum=Instruction('~',"Input Character","Pushes character from user input.")
@inNum.runner
def run(instr,funge,pointer):
...
# Concurrency
split=Instruction('t',"Split","Duplicates instruction pointer with an opposite delta.")
@split.runner
def run(instr,funge,pointer):
